from rawDataBase import rawDataIOBase, rawData
from rawDeezerAPIData import deezerAlbum, deezerArtist, deezerTrack
from dbArtistID import dbArtistIDDeezer, dbArtistIDModValue

class rawDeezerDataIO(rawDataIOBase):
    def __init__(self, debug=False):
        super().__init__()
        self.aid = dbArtistIDDeezer()
    ...
    def get(self, inputdata):
        ...
        for artistID,artistIDData in artistsData.items():            
            ...
            for trackID,trackData in artistIDData["Tracks"].items():
                ...
                amdc = self.makeRawMediaReleaseData(album=trackName, url=trackURL, artist=artistID, code=trackID, year=None, aclass=None, aformat={"Type": trackType})
                if mediaData.get(mediaType) is None:
                    mediaData[mediaType] = []
                mediaData[mediaType].append(amdc)
